...
$n=1,000$ extraídas de una distribución uniforme con los mismos parámetros. Se observa que las formas de las muestras no siempre coinciden exactamente con la distribución original, lo que evidencia la variabilidad natural del muestreo. Sin embargo, los promedios muestrales tienden a aproximarse al valor teórico de la media de 0.5.
...
La función de densidad de probabilidad de una variable aleatoria $X$ que sigue una distribución normal, denotada como $X \sim N(\mu, \sigma^2)$, está dada por:  
...
```
**Figura 2.30** Distribución de 12 muestras aleatorias (tamaño 1,000) de una población **normal** con media 200 y varianza 50.
</center>
<br/><br/>
...
La función de **densidad de probabilidad** de una variable aleatoria \(X\) con distribución **exponencial** de parámetro \(\lambda > 0\) se define como:  
...
La función de distribución acumulada, que describe la probabilidad de que la variable aleatoria tome un valor menor o igual a \( x \), se expresa como:
...
# Crear la gráfica de la función de probabilidad para diferentes valores de lambda
...
# Generar datos para la función de distribución acumulada de la exponencial
data <- expand.grid(x = x_vals, lambda = lambdas) %>%
  mutate(FDA = pexp(x, rate = lambda))

# Crear la gráfica de la función de distribución acumulada para diferentes valores de lambda
...
La distribución de Weibull es ampliamente utilizada en diversas disciplinas, incluyendo:  
...
<h3> Distribución de Weibull</h3>

La distribución de Weibull se define mediante los siguientes parámetros de forma   \( a \) y de escala \( \sigma \).
...
- Cuando \( a = 1 \), la distribución de **Weibull** se reduce a la **distribución exponencial** con parámetro \( \sigma \).  
...
<h3> Función de riesgo en la distribución de Weibull  </h3>
...
La función de riesgo de la distribución de Weibull depende del **parámetro de forma \( a \)**:
...
>
<p>
En esta actividad se utilizan las funciones de **R** asociadas a la distribución de **Weibull** para explorar su comportamiento mediante tres enfoques clave: 
</p>
...
<br/>
- **Generación de una muestra aleatoria**: Obten datos simulados a partir de la distribución de Weibull y analiza sus propiedades.
<br/>
