---
title: <span style="color:#686868"> **Modelos continuos**</span>
...
- **Valor esperado (media):**  
  ...
  E[X] = \frac{a + b}{2}
  $$  
  La media representa el punto medio del intervalo, lo que refleja la simetría de la distribución.  

- **Varianza:**  
  $$
  \text{Var}(X) = \frac{(b - a)^2}{12}
  $$  
  La varianza indica la dispersión de los valores alrededor de la media y depende únicamente de la amplitud del intervalo.  
...
plot.unif1<-ggplot() + 
  ...
  labs(title = "Función de Densidad de Probabilidad de la Distribución Uniforme",
       x = "Valor de X",
...
La media teórica de una variable aleatoria \( X \sim U(a, b) \) se calcula con la siguiente fórmula:
...
Para la **distribución Uniforme continua** en el intervalo \( [a, b] \), la función de densidad de probabilidad (FDP) es:
...
plot.unif<-ggplot(data, aes(x = x)) +
  ...
  labs(title = "Muestras Aleatorias de la Distribución Uniforme (0,1)",
       x = "Valor de X",
       y = "Frecuencia") +
  theme_minimal() +
...
- **Máxima entropía:** Entre todas las distribuciones con una media y varianza dadas, la normal es la que tiene mayor entropía, lo que la hace la más dispersa posible dentro de estos parámetros.  

- **Suma de normales:** Si $X_1 \sim N(\mu_1, \sigma_1^2)$ y $X_2 \sim N(\mu_2, \sigma_2^2)$ son independientes, entonces $X_1 + X_2$ sigue una distribución normal con parámetros:  
  $$
...
# Crear la gráfica
fig_normal<- ggplot(data, aes(x = x, y = densidad, color = as.factor(varianza))) +
  geom_line(size = 1) +
...
plot_normal2 <- ggplot(data, aes(x = x, y = densidad, color = as.factor(media))) +
...
fig.hist.muestra <- ggplot(data_long, aes(x = x)) +
...
Podemos calcular esta integral en **R** usando la función `integrate()`:
...
También podemos verificar el resultado usando la función `pnorm()` en **R**:
...
En todos los casos presentados, la probabilidad obtenida es **0.9522**. Además, la **Figura 2.31** ilustra el área bajo la curva que representa la probabilidad \( P(X > 150) \), cuyo valor es **0.9522**.
...
fig_area<-ggplot(data.frame(x = x_vals), aes(x = x)) +
...
La **Figura 2.32** ilustra el área bajo la curva que representa la probabilidad \( P(Z > -1.67) \), cuyo valor es **0.9522**.
...
fig_area_Z <- ggplot(data.frame(x = x_vals), aes(x = x)) +
  stat_function(fun = dnorm, args = list(mean = mu, sd = sigma), 
                linewidth = 1, color = "blue") +  
...
No. La regla empírica es válida para **cualquier distribución normal** independientemente de sus parámetros \(\mu\) y \(\sigma\).  
...
La función de **densidad de probabilidad (PDF)** de una variable aleatoria \(X\) con distribución **Exponencial** de parámetro \(\lambda > 0\) se define como:  
...
Algunos autores utilizan la notación alternativa \( \lambda = \frac{1}{\beta} \), donde \( \beta \) se interpreta como el **tiempo medio entre eventos**.
...
1. **Media:**  
   ...
   E[X] = \frac{1}{\lambda}
   $$
   Lo que indica que el valor promedio de la variable aleatoria es inversamente proporcional a la tasa de ocurrencia de eventos.

2. **Varianza:**  
   $$
   \text{Var}(X) = \frac{1}{\lambda^2}
   $$
   Esto sugiere que cuanto mayor sea \( \lambda \), menor será la dispersión de los valores.
...
4. **Relación con la Distribución de Poisson:**  
   - Si los eventos ocurren de acuerdo con un **proceso de Poisson** con tasa \( \lambda \), entonces los tiempos entre eventos consecutivos siguen una distribución **exponencial con parámetro \( \lambda \)**.
...
ggplot(data, aes(x = x, y = FDP, color = as.factor(lambda))) +
  ...
  labs(title = "Función de Densidad de Probabilidad de la Distribución Exponencial",
       x = "Valor de X",
       y = "f(X) = λ e^{-λX}",
       color = "Lambda (λ)") +
  theme_minimal()
...
# Crear la gráfica de la FDP para diferentes valores de lambda
plot.exp<-ggplot(data, aes(x = x, y = FDP, color = as.factor(lambda))) +
  geom_line(linewidth = 1) +
  labs(title = "Función de Densidad de Probabilidad de la Distribución Exponencial",
       x = "Valor de X",
       y = "f(X) = λ e^{-λX}",
...
ggplot(data, aes(x = x, y = FDA, color = as.factor(lambda))) +
  geom_line(linewidth = 1) +
  labs(title = "Función de Distribución Acumulada de la Distribución Exponencial",
       x = "Valor de X",
       y = "F(X) = P(X ≤ x)",
       color = "Lambda (λ)") +
  theme_minimal()
...
# Crear la gráfica de la FDA para diferentes valores de lambda
plt.fda.exp<-ggplot(data, aes(x = x, y = FDA, color = as.factor(lambda))) +
  geom_line(linewidth = 1) +
  labs(title = "Función de Distribución Acumulada de la Distribución Exponencial",
       x = "Valor de X",
       y = "F(X) = P(X ≤ x)",
...
La distribución **Gamma** se define mediante dos parámetros, uno de forma  \( \alpha \) y otro de escala \( \beta \), ambos positivos. La función de densidad de probabilidad  de una variable aleatoria \( X \sim Gamma(\alpha, \beta) \)   está dada por:  


$$
f(x) =
\begin{cases} 
\frac{\beta^\alpha x^{\alpha - 1} e^{-\beta x}}{\Gamma(\alpha)}, & x > 0 \\[10pt]
0, & x \leq 0 
\end{cases}
$$
...
En **R**, la distribución Gamma se parametriza de manera diferente mediante los parámetros  de forma \( \alpha \) y escala \( \sigma \), ambos positivos. La función de densidad de probabilidad  de una variable aleatoria \( X \sim \text{Gamma}(\alpha, \sigma) \) en esta parametrización es:
...
plot.gamma<-ggplot(data, aes(x = x, y = densidad, color = label)) +
  geom_line(linewidth = 1) +
...
# Definir valores de parámetros de la distribución Gamma
parametros <- data.frame(
  alpha = c(1,1,1,1),  # Valores del parámetro de forma (shape)
  sigma = c(1,1/2,1/4,1/5),  # Valores del parámetro de tasa (rate)
  color = c("#FF7F00", "#034A94", "#686868", "#76EEC6")  # Colores asociados
)
...
- **Coeficiente de variación:**  
  \[
  \frac{\sigma}{E[X]} = \frac{\sqrt{V[X]}}{E[X]}
  \]
...
1. **Si \( a < 1 \) (Distribución con tasa de falla decreciente)**  
   ...
   - Ejemplo: Productos con fallas tempranas por defectos de fabricación (fase de mortalidad infantil en análisis de confiabilidad).  
...
2. **Si \( a = 1 \) (Distribución Exponencial, tasa de falla constante)**  
   ...
   - Ejemplo: Componentes electrónicos que fallan con una probabilidad constante.  
...
3. **Si \( a > 1 \) (Distribución con tasa de falla creciente)**  
   ...
   - Ejemplo: Motores, rodamientos y estructuras sometidas a fatiga.  
...
- **Visualización de la función de densidad**: Graficaremos la curva de densidad de una variable aleatoria \( X \sim \text{Weibull}(a = 1, \sigma = \pi) \).  
-  **Cálculo del área bajo la curva**: Determinaremos la probabilidad acumulada en un intervalo específico.  
- **Generación de una muestra aleatoria**: Obtendremos datos simulados a partir de la distribución Weibull y analizaremos sus propiedades.
- Usa `dweibull()`, `pweibull()` y `rweibull()`
</p>
>
</div>
