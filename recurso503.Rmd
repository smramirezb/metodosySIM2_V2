...
| **Diferencia de Medias (Muestras Dependientes) \( \mu_D \)** | Muestras pareadas, diferencias distribuidas normalmente | \( \mu_D = d_0 \) | \( T = \frac{\bar{d} - d_0}{\frac{s_d}{\sqrt{n}}} \sim t_{(v=n-1)} \), donde \( \bar{d} = \frac{1}{n} \sum_{i=1}^{n} d_i \) y \( d_i = x_{1i} - x_{2i} \) |
...
El **valor-p** obtenido es **1.088e-07**, lo que, con un nivel de significancia del 1%, permite inferir que las **diferencias no siguen una distribución normal**.
...
- **Grupo 1**: \( \text{Valor-p} = 0.3018 \)
- **Grupo 2**: \( \text{Valor-p} = 0.6424 \)
...
Dado que el valor-p es menor que 2.2e-16, el cual es significativamente inferior al nivel de significancia de 0.05, se rechaza la hipótesis nula. Esto indica que existe evidencia estadísticamente significativa para concluir que la media del grupo 1 es menor que la media del grupo 2. 
...
Respecto al test de medias, el valor-p obtenido es 3.781e-12, que es menor que 0.05. Por lo tanto, se rechaza la hipótesis nula y se acepta la hipótesis alternativa, lo que indica que la media de los índices del grupo 2 es significativamente mayor que la del grupo 3.
...
1. **El tamaño del efecto (\( d \))**: Cuanto **mayor** es la diferencia entre las medias poblacionales en relación con la variabilidad de los datos, **mayor será la potencia** de la prueba.
...
A continuación, se presentan los resultados de la simulación para distintos tamaños del **efecto de Cohen (\(d\))**, los cuales se categorizan de la siguiente manera:  
...
Para evaluar la influencia del **tamaño del efecto (\(d\))** en la **potencia estadística**, se modela su relación manteniendo constante el **nivel de significancia** (\(\alpha = 0.05\)). Este análisis permite comprender cómo la magnitud del efecto influye en la capacidad de una prueba estadística para detectar diferencias significativas entre grupos.
