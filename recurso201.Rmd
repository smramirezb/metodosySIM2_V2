...
<h3>Función de distribución de probabilidad (fdp)</h3>
...
- La fda toma valores  en el intervalo [0, 1], ya que expresa probabilidades acumuladas.
...
A continuación, se presentan las propiedades fundamentales de la **función de densidad de probabilidad (fdp)**, la cual describe la distribución de una variable aleatoria continua:

- **No negatividad**: La fdp siempre toma valores no negativos, es decir:
  \[ f(x) \geq 0, \quad \text{para todo } x. \]

- **Área total bajo la curva**: La integral de la fdp en todo el dominio debe ser igual a 1, lo que asegura que la probabilidad total sea completa:
  \[ \int_{-\infty}^{\infty} f(x) \, dx = 1. \]

- **Probabilidad como área bajo la curva**: La probabilidad de que la variable aleatoria \( X \) tome un valor dentro del intervalo \([a, b]\) se obtiene mediante la integral de la fdp en dicho intervalo:
...
>
<p>
En el caso de variables continuas, La fdp es la derivada de la función de distribución acumulada (fda): \[ f(x) = \frac{d}{dx}F(x) \]
</p>
>
</div>  
