...
Además, los valores del **promedio** y la **varianza** de las 1000 sumas obtenidas (\(\bar{S} = 199.4924\) y \(s^2 = 382.5936\)) presentan una **gran proximidad** a los valores teóricos esperados \(\mathbb{E}[S] = n\mu=(100)(2)=200\) y \(\text{Var}(S) = n\sigma^2=(100)(4)=400\). Esto sugiere que:
...
- **Criterio de decisión**:
  - Si el **p-valor \( p \) es mayor que $\alpha = 0.05$**, *no se rechaza \( H_0 \)*, lo que sugiere que los datos **podrían seguir** una distribución normal.
  - Si el **p-valor \( p \) es menor que $\alpha = 0.05$**, *se rechaza \( H_0 \)* en favor de \( H_A \), indicando que los datos **no siguen** una distribución normal.
...
Los **p-valores** obtenidos en los tests fueron:

- **Shapiro-Wilk:** \( p-valor = 0.08331 \)
- **Kolmogorov-Smirnov:** \( p-valor = 0.7038 \)

Dado que en ambos casos el **p-valor es mayor a \(\alpha = 0.05\)**, **no se rechaza la hipótesis nula**, lo que indica que las sumas muestrales pueden considerarse como provenientes de una distribución normal con un nivel de significancia del **5%** ($\alpha = 0.05$).
...
for (n in tamanos_muestras) {
  ...
  cat("Test de Shapiro-Wilk: p-valor =", shapiro_test$p.value, "\n")
  cat("Test de Kolmogorov-Smirnov: p-valor =", ks_test$p.value, "\n")
}
...
Test de Shapiro-Wilk: p-valor = 2.780042e-16 
Test de Kolmogorov-Smirnov: p-valor = 0.0006052599 

-----------------------------------
Resultados para n = 10 
Promedio de sumas muestrales: 20.14694 
Varianza de sumas muestrales: 40.20084 
Test de Shapiro-Wilk: p-valor = 4.53899e-09 
Test de Kolmogorov-Smirnov: p-valor = 0.08104603 

-----------------------------------
Resultados para n = 80 
Promedio de sumas muestrales: 159.3497 
Varianza de sumas muestrales: 312.4678 
Test de Shapiro-Wilk: p-valor = 0.03527495 
Test de Kolmogorov-Smirnov: p-valor = 0.2258247 

-----------------------------------
Resultados para n = 200 
Promedio de sumas muestrales: 398.6901 
Varianza de sumas muestrales: 787.8698 
Test de Shapiro-Wilk: p-valor = 0.02210118 
Test de Kolmogorov-Smirnov: p-valor = 0.4179235 

-----------------------------------
Resultados para n = 500 
Promedio de sumas muestrales: 1001.354 
Varianza de sumas muestrales: 1987.363 
Test de Shapiro-Wilk: p-valor = 0.8243656 
Test de Kolmogorov-Smirnov: p-valor = 0.9836838 

-----------------------------------
Resultados para n = 2000 
Promedio de sumas muestrales: 3998.977 
Varianza de sumas muestrales: 7882.959 
Test de Shapiro-Wilk: p-valor = 0.6293026 
Test de Kolmogorov-Smirnov: p-valor = 0.6349088 
</pre>
---
...
\[
\mathbb{E}[S] = n\mu, \quad \text{Var}(S) = n\sigma^2.
\]
...
- Para \( n = 5 \):  
  ...
  Valores esperados:  
  \[
  \mathbb{E}[S] = 10, \quad \text{Var}(S) = 20.
  \]
...
- Para \( n = 2000 \):  
  ...
  Valores esperados:  
  \[
  \mathbb{E}[S] = 4000, \quad \text{Var}(S) = 8000.
  \]
...
- **\( n = 5 \) y \( n = 10 \)**
  - Ambos test presentan **p-valores menores a 0.05**, indicando que las sumas **no siguen una distribución normal**.
  - Esto era **esperado**, ya que con muestras pequeñas el Teorema del Límite Central (TLC) no garantiza normalidad.
  - El **p-valor extremadamente bajo en Shapiro-Wilk** sugiere que la distribución de las sumas aún conserva la asimetría de la distribución exponencial original.

- **\( n = 80 \)**
  - **Kolmogorov-Smirnov** muestra un **p-valor de 0.2258**, lo que **no permite rechazar la normalidad**.
  - **Shapiro-Wilk**, en cambio, presenta un **p-valor de 0.035**, lo que **rechaza la normalidad a nivel \( \alpha = 0.05 \)**.
...
- **\( n = 200 \)**
  - Similar al caso anterior:
    - **Shapiro-Wilk** da un **p-valor de 0.022**, lo que indica **rechazo de la normalidad**.
    - **Kolmogorov-Smirnov** da un **p-valor de 0.4179**, indicando que no hay suficiente evidencia para rechazar la normalidad.
...
- **\( n = 500 \) y \( n = 2000 \)**
  - En estos casos, **ambos test no rechazan la normalidad** (\( p \gg 0.05 \)).
